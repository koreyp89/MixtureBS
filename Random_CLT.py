...
class CLT:
    ...
    def learn(self, dataset, r):
        ...
        num = r / 100
        num = math.ceil(num*edgemat.shape[0]*edgemat.shape[1])
        for i in range(num):
            row = np.random.randint(0, edgemat.shape[0]-1)
            col = np.random.randint(0, edgemat.shape[1]-1)
            edgemat[row][col] = 0
